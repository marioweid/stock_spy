...
def _report(**over: object) -> EquityReport:
    defaults: dict[str, Any] = {
        ...
        "opportunity_note": "Decent entry near support.",
    }
    defaults.update(over)
    return EquityReport(**defaults)


def _bundle(*, price: float, holding: Holding | None, cash: float) -> ReportBundle:
    quote = Quote(
...
def test_summary_has_score_verdict_and_metrics() -> None:
    msg = format_summary(_bundle(price=100.0, holding=None, cash=0.0))
...
def test_protection_alert_quantifies_money_at_risk() -> None:
    ...
    msg = format_protection_alert(_bundle(price=90.0, holding=holding, cash=0.0))
...
def test_protection_alert_falls_back_without_holding() -> None:
    msg = format_protection_alert(_bundle(price=100.0, holding=None, cash=0.0))
    assert "/100" in msg  # falls back to summary


def test_opportunity_alert_shows_full_trade_plan() -> None:
    msg = format_opportunity_alert(_bundle(price=100.0, holding=None, cash=10_000.0))

    assert "SWING IDEA" in msg
...
def test_markdown_has_sections_and_glossary() -> None:
    md = render_markdown(
        _bundle(
            price=100.0, holding=Holding(ticker="AAPL", shares=10, avg_cost=80.0), cash=10_000.0
        )
    )
