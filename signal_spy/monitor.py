"""The monitor cycle: protect holdings on major price/news events and persist state."""
...
from signal_spy.analysis import analyze as default_analyze
from signal_spy.models import AppConfig, EquityReport, Fundamentals, Holding
from signal_spy.providers.fundamentals import get_fundamentals as default_get_fundamentals
from signal_spy.providers.prices import QuoteUnavailableError
from signal_spy.providers.prices import get_quote as default_get_quote
from signal_spy.report import (
    format_protection_alert,
    format_summary,
    render_markdown,
)
from signal_spy.store import PriceBaseline, Store
...
class Monitor:
    ...
    async def _process(self, target: _Target) -> int:
        """Evaluate one ticker and alert if it needs protection or moved past its threshold."""
        try:
    ...
    async def _build_bundle(
        ...
        return assemble_report(quote, fundamentals, technicals, report, holding=target.holding)
...
def _decide_alert(bundle: ReportBundle, price_alert: bool) -> tuple[str, str] | None:
    """Pick the alert to send, in priority order: protect, then big move."""
