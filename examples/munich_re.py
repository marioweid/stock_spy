"""Example: run a swing analysis on Munich Re (MUV2.DE) and send the result to Telegram.

Run with::

    uv run python examples/munich_re.py
...
TICKER = "MUV2.DE"  # Munich Re on Xetra
NEWS_CONTEXT = 8
HTTP_TIMEOUT = 30.0

logger = logging.getLogger("munich_re")
...
async def main() -> None:
    """Analyse Munich Re once and deliver the outcome to Telegram."""
    ...
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
        ...
        if evaluated is None:
            await send_message(
                token,
                chat,
                f"{TICKER} (Munich Re): no clean swing setup right now.",
                client=client,
            )
        ...
        await send_document(
            ...
            caption=f"Swing analysis: {TICKER} (Munich Re)",
            client=client,
        )
