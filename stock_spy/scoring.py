"""Deterministic scoring: the quality indicator, protection engine, and swing engine.
...
def size_position(
    ...
    *,
    ...
    """Size a swing trade so a stop-out loses at most ``risk_per_trade_pct`` of cash.

    Shares are also capped so the position costs no more than ``max_position_pct`` of cash.
    Returns ``None`` when the math yields no affordable, positive-risk position.
    """
    risk_per_share = entry - stop
    if risk_per_share <= 0 or cash <= 0 or entry <= 0:
        return None
    shares_by_risk = math.floor((cash * risk_per_trade_pct / 100.0) / risk_per_share)
    shares_by_cost = math.floor((cash * max_position_pct / 100.0) / entry)
    ...
    return TradePlan(
        ...
        pct_of_cash=round(cost / cash * 100.0, 1),
    )
