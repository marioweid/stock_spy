...
Every metric shown is paired with a one-line explanation from :mod:`stock_spy.glossary`, so a
...
from stock_spy.analysis import ReportBundle
from stock_spy.glossary import GLOSSARY, explain
from stock_spy.models import RiskLevel, Signal
...
def format_opportunity_alert(bundle: ReportBundle) -> str:
    """Render a full swing trade plan for a watchlist setup (HTML)."""
    q, swing, plan = bundle.quote, bundle.swing, bundle.trade_plan
    if not swing.is_setup or plan is None:
        return format_summary(bundle)
    cur = q.currency
    lines = [
        f"\U0001f4a1 <b>SWING IDEA: {escape(q.ticker)}</b>",
        escape(swing.rationale),
        "",
        "<b>Trade plan (sized from your cash)</b>",
        f"• Buy zone: {swing.entry_low:.2f} to {swing.entry_high:.2f} {escape(cur)}",
        f"• Stop-loss: {plan.stop:.2f} {escape(cur)} (exit here to cap the loss)",
        f"• Target: {plan.target:.2f} {escape(cur)}",
        f"• Risk/Reward: {plan.risk_reward:g} (aim to make {plan.risk_reward:g}x the risk)",
        f"• Size: {plan.shares} shares ≈ {plan.cost:,.2f} {escape(cur)} "
        f"({plan.pct_of_cash:g}% of cash)",
        f"• If stopped out: ≈ {plan.risk_amount:,.2f} {escape(cur)} loss; "
        f"at target: ≈ {plan.reward_amount:,.2f} {escape(cur)} gain.",
        "",
        escape(bundle.report.opportunity_note),
        "",
        "Not financial advice. Always set your stop before you buy.",
    ]
    return "\n".join(lines)
...
def render_markdown(bundle: ReportBundle) -> str:
    ...
    out += ["", "## Swing setup", *_markdown_swing(bundle, cur)]
...
def _markdown_swing(bundle: ReportBundle, cur: str) -> list[str]:
    """The swing-setup section: a sized trade plan, or a stand-aside note."""
    swing, plan = bundle.swing, bundle.trade_plan
    if not swing.is_setup:
        return [swing.rationale or "No clean swing entry right now."]
    lines = [swing.rationale]
    if plan is not None:
        lines += [
            f"- Buy zone: {swing.entry_low:.2f} to {swing.entry_high:.2f} {cur}",
            f"- Stop-loss: {plan.stop:.2f} {cur}",
            f"- Target: {plan.target:.2f} {cur} (risk/reward {plan.risk_reward:g})",
            f"- Size from cash: {plan.shares} shares ≈ {plan.cost:,.2f} {cur} "
            f"({plan.pct_of_cash:g}% of cash); risk ≈ {plan.risk_amount:,.2f} {cur}.",
        ]
    else:
        lines.append("_Set up looks valid but no cash configured to size a trade._")
    lines += ["", bundle.report.opportunity_note]
    return lines
